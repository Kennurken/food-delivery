...
| GET/POST | /api/v1/orders/{id}/messages | same as the ticket |
...
| PUT/DELETE | /api/v1/me/devices | user |
...
Live updates: `WS /api/v1/ws?token=<jwt>` streams `{"type":"order.updated","cause":"status"|"location","order":{...}}` and `{"type":"order.chat","message":{...}}`. Location and chat do not toast as status changes. Couriers also get unassigned **delivery** pickable orders. Hub is in-process — one instance; swap for Redis pub/sub to scale out.
